...
1. **单一事实源 = 本仓库**
   Claude 会话记忆是易失的；仓库才是持久的。所有重要决策（架构、契约、约定）必须落到仓库文件：本 `CLAUDE.md`、`docs/design/` 设计文档、`docs/` 开发指南、`src/.../schemas/`。**不要把关键信息只留在会话里。**

2. **契约先行（Contract-First）—— 这是并行开发的核心范式**
   - 动手实现任何节点前，**先在 `src/cc_bom_generator/schemas/` 定义并提交该节点的输入/输出 Pydantic schema**。
...
4. **开工先 `git pull`，按模块分工**
   ...
   - **标准开工触发器**：当开发者在终端输入带有"开工"、"开始开发"、"今天继续"等意图的指令时，Claude Code 必须自动按顺序串行执行以下 3 步，无需等待确认：
     ...
     ③ 读取 `docs/技术设计-生成场景后端专项.md` 和 `schemas/`，对比是否有影响当前模块的变更。全部完成后，向开发者输出简报再开始写代码。
...
7. **开工前必须读技术文档（即使以前读过）**
   ...
   - `backend/src/cc_bom_generator/schemas/` —— 契约定义（即使没变也要确认）
   这一步不可跳过。**不读就直接改代码属于违规操作，会被 review 时打回**。

8. **进度跟踪纪律（代码改完必须同步文档，无例外）**
   - **CC 硬性规则**：每次 `git commit` 之前，**必须**先完成以下 3 项同步，否则不允许 commit：
     ① **更新 `docs/progress.md`**：在"已完成"表里加一行（日期/模块/文件/说明），更新"当前任务"和"下一步"
     ② **检查 `docs/技术设计-生成场景后端专项.md` 是否受影响**：如果改了接口/契约/表结构/算法/文件结构，必须在对应章节同步修改
     ③ **检查 `CLAUDE.md` 目录结构和协作矩阵是否受影响**：如果新增/删除/移动了文件，必须更新目录树
   - 提交时必须显式标注改动的模块编号和文件路径。
   - 林大宇和杨力各自维护自己的进度段落，**不覆盖对方的内容**。
   - 每次开工前先读 `docs/progress.md`，确认对方当前进度后再开始写代码。
   - **这条没有例外。commit 前不同步文档 = 违规操作。**

9. **改契约 = 停止 + 通知**
   - 任何涉及 `src/cc_bom_generator/schemas/` 的改动，必须先停止代码工作，通知对方（发消息或开 PR），等对方确认后再继续。
   - 这条没有例外。**契约是协作边界，改契约不通知等于拆桥**。
   - **Auto 模式熔断器**：即使 Claude Code 当前运行在 auto/acceptEdits 模式，一旦判定需要修改 `src/.../schemas/` 或 `backend/src/cc_bom_generator/schemas/` 目录下的契约文件，CC 必须**强行挂起（Suspend）当前自动流程**。向终端抛出高亮警告说明原因，并强制等待开发者输入明确授权（如"同意修改"）后才能继续。严禁静默修改契约。
...
│  │  ├─ enums/                # ✅ 枚举唯一事实源（BomSource/BOMStatus/DiagnosisCategory/ModificationType 等 12 个）
│  │  ├─ schemas/              # Pydantic 契约（9 个文件，含 bom_delta.py；引用 enums 不内联枚举）
│  │  ├─ nodes/                # 编排层（base/orchestrator/pipeline/skills）
│  │  ├─ db/                   # ORM（13 张表，含 PendingDelta）+ Repository + Alembic
│  │  ├─ api/                  # HTTP 层（routers/generate.py + deps.py）
│  │  ├─ services/             # 业务编排层（generate_service.py + ingest_service.py，事务边界）
│  │  ├─ llm/                  # LLM 客户端（双格式 OpenAI/Anthropic）
│  │  ├─ logging_config.py     # 统一日志（log.info/log.error）
│  │  └─ app.py                # create_app() 工厂（分层详见技术文档第10章）
│  ├─ main.py                  # 启动薄壳（src/main.py，uvicorn src.main:app）
│  ├─ requirements.txt
...
schemas/                            林大宇    ❌      Pydantic 契约，改=PR+通知
...
services/generate_service.py           林大宇    ❌      业务编排（HTTP 事务边界：run_generate）
services/ingest_service.py             ❌        杨力    A模块：Excel解析→CleanedTestSet（杨力接管）
main.py（src/薄壳）                    林大宇    ❌      启动入口（uvicorn src.main:app）
alembic/                              林大宇    ❌      迁移脚本
...
- [x] 开发文档（协作指南 + 模块拆分）
- [x] schemas 定义（9 个 Pydantic 文件，含 bom_delta.py）
- [x] nodes/generate 节点实现（关键词→定义→规则→画像→校验→自检→组装 + 回修）
- [x] db/models + Alembic 迁移（13 张表 + 0001/0002/0003；PR1 加 PendingDelta + badcases.trace_json）
- [x] llm/client 客户端（双格式 OpenAI/Anthropic）
- [x] Step 2 分层重构（schemas 搬移 + PipelineRepository + api/services 分层）
- [ ] **调优闭环**（diagnose / optimize / apply / evaluate）← 下一步重点
- [ ] nodes/ingest（A 模块/杨力：Excel→CleanedTestSet 完整版，替简版）
- [ ] Vue3 前端（工作台/调优成效/设置）
- [ ] 集成联调 + 回归测试
